...
IndexPtr parse_index(long offset, int dbase, char *line) {
    ...
    idx->wd = new char [strlen(ptrtok) + 1];
    assert(idx->wd);
    strcpy(idx->wd, ptrtok);
    
    /* get the part of speech */
    ptrtok=strtok(NULL," \n");
    idx->pos = new char [strlen(ptrtok) + 1];
    assert(idx->pos);
...
SynsetPtr parse_synset(FILE *fp, int dbase, char *word)
{
    ...
    /* looking at POS */
    ptrtok = strtok(NULL, " \n");
    synptr->pos = new char[strlen(ptrtok) + 1];
    assert(synptr->pos);
	...
	ptrtok = strtok(NULL, " \n");
	synptr->words[i] = new char[strlen(ptrtok) + 1];
	...
	synptr->defn = new char[strlen(tbuf) + 4];
	assert(synptr->defn);
...
static char *printant(int dbase, SynsetPtr synptr, int wdnum, char *templat, char *tail)
{
    int i, j, wdoff;
    SynsetPtr psynptr;
    char tbuf[WORDBUF];
	...
	if (synptr->ptrtyp[i] == ANTPTR && synptr->pfrm[i] == wdnum) {
		...
		if (psynptr->ptrtyp[j] == ANTPTR &&
			...
			first = 0;
		    sprintf(retbuf + strlen(retbuf), templat, tbuf);
		}
	    }
	    free_synset(psynptr);
	}
