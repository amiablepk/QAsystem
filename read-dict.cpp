...
Dict_node * insert_dict(Dictionary dict, Dict_node * n, Dict_node * newd) {
...
/* and right fields of it are NULL                                       */
    ...
    if (n == NULL) return newd;
    comp = dict_compare(newd->string, n->string);
    if (comp < 0) {
        n->left = insert_dict(dict, n->left, newd);
    } else if (comp > 0) {
        n->right = insert_dict(dict, n->right, newd);
    } else {
...
void set_parent_of_node(Dictionary dict, 
			Dict_node *p, Dict_node * del, Dict_node * newd) {
    if (p == NULL) {
	dict->root = newd;
    } else {
	if (p->left == del) {
